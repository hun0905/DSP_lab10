% this script has test cases to help you test my_imfilter.m which you will
% write. You should verify that you get reasonable output here before using
% your filtering to construct a hybrid image in generate_hybrid_image.m. All outputs are
% saved and you can include them in your writeup. You can add calls to
...
test_image = imresize(test_image, 0.7, 'bilinear'); %resize to speed up testing
figure(1)
imshow(test_image)
...
blur_filter = blur_filter / sum(sum(blur_filter)); %make the filter sum up to 1
...
large_blur_image = my_imfilter(large_blur_image, large_1d_blur_filter'); %notice the ' operator transposes the filter
